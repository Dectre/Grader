...
# Custom CSS matching PyQt6 Application UI
...
def show_pdf(file_path):
    if file_path and os.path.exists(file_path):
        ...
        pdf_display = f'<object data="data:application/pdf;base64,{base64_pdf}" type="application/pdf" width="100%" height="900px"><p>Your browser does not support inline PDFs.</p></object>'
        st.markdown(pdf_display, unsafe_allow_html=True)
    else:
        st.warning("📄 No PDF file uploaded for this student.")

# ------------------- Top Navigation Bar -------------------
top_col1, top_col2, top_col3, top_col4 = st.columns([1, 4, 1, 1])
...
# ------------------- Main Layout (PDF Viewer + Grading Panel) -------------------
...
with col_pdf:
    ...
    with pdf_top_right:
        if student['pdf'] and os.path.exists(student['pdf']):
            ...
            # دکمه با قابلیت باز شدن در New Tab
            pdf_link_html = f'<a href="data:application/pdf;base64,{b64_pdf}" target="_blank" class="open-tab-btn">↗️ Open in New Tab</a>'
...
with col_grading:
    ...
    with st.form(key=f"grading_form_{student['id']}"):
        ...
        # اصلاح نمایش نمره توتال بدون کد خام
